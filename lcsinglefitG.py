...
def vincfit(lctuple, p0s=[], initsteps=20, nclusters=30, threshratio=2, method='BFGS', plsprint='some', plsplot=False):
  ...
  opts,sses,p0s = vilcfits(phase,intensity,p0s,initsteps,meth=method)
  ...
  #full fits
  opts,sses,p1s = vilcfits(phase,intensity,newp0s,meth=method)
...
def fincfit(lctuple, inc, p0s=[], initsteps=20, nclusters=20, threshratio=2, method='BFGS', plsprint='some', plsplot=False, snum=-1):
  phase, intensity = lctuple
  ...
  #initial fits using leastsq, starting at various points in param space
  opts,sses,p0s = filcfits(phase,intensity,inc,p0s,initsteps,meth=method)
  ...
  #full fits
  opts,sses,p1s = filcfits(phase,intensity,inc,newp0s,meth=method)
...
def vilcfits(phase,intensity,p0s=[],mfev=None,meth='BFGS'):
  if not p0s:
    p0s = spacedparams(5)
  opts = []
  sses = []
  p0s2 = []
  if mfev:
    optns = {'maxiter': mfev, 'disp': False}
  else:
    optns = {'disp': False}
  for p0 in p0s:
    try:
      res = minimize(lcspotsse3, p0, args=(phase,intensity), method=meth, options=optns)
      opts.append(res.x)
      sses.append(lcspotsse(res.x,phase,intensity))
      p0s2.append(p0)
...
def filcfits(phase,intensity,inc,p0s=[],mfev=None,meth='BFGS'):
  if not p0s:
  ...
  for p0 in p0s:
    try:
      res = minimize(lcspotssefi3, p0, args=(phase,intensity,inc), method=meth, options=optns)
      opts.append(res.x)
      sses.append(lcspotssefi(res.x,phase,intensity,inc))
      p0s2.append(p0)
    except RuntimeError as e:
      print "Runtime Error({0}): {1}".format(e.errno, e.strerror)
  return opts,sses,p0s2
